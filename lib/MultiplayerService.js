...
class MultiplayerService {
  ...
  async createRoom(roomCode) {
    try {
      ...
      const attempts = [
        { code: roomCode, host_user_id: user.id, state: 'waiting', random_seed: randomSeed },
        { code: roomCode, host_user_id: user.id, state: 'waiting' },
        { code: roomCode, host_id: user.id, state: 'waiting', random_seed: randomSeed },
        { code: roomCode, host_id: user.id, state: 'waiting' },
  ...
  async addPlayer(playerName, avatarColor) {
    try {
      ...
      if (!success) throw lastError || new Error('Failed to add player');
      return { success: true };
    } catch (error) {
